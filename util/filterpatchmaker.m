function [patches] = filterpatchmaker(nums, numt, shifts, shiftt, hbw, filter)
    ...
    for l=1:2*hbw+1
      for k=1:2*hbw+1
        ...
        if(~(i_ <= 0 || i_ > nums || j_ < 0 || j_ >= numt))
          ...
          js(winsize*(tileptr-1)+tcolptr) = tileptr;
          vs(winsize*(tileptr-1)+tcolptr) = filter(tcolptr);
        end
...
patches = (sparse(is, js, vs, npix, npix))';
