function [b_img2, b_img2_masked, b_img_transpose_filt, nwmat, mask] = recon_matrix_construction(image, mask, pyr, forest_model, params, subsample)
...
mask_subsample = false(size(mask));
mask_subsample(subsample(1):subsample(2), subsample(3):subsample(4)) = true;
mask_subsample = mask & mask_subsample;

mask_portion = mask(subsample(1):subsample(2), subsample(3):subsample(4));

nums = subsample(2)-subsample(1)+1;
numt = subsample(4)-subsample(3)+1;

% Get the 2D pointer as well as the linear pointer.
[iptrs, jptrs] = find(mask_portion);
%[iptrs, jptrs] = find(mask);
ptrs = find(mask);

gegfmat = pyr_to_feature_vec(pyr);

[gegam, gegbv] = get_M_and_B(forest_model, gegfmat(ptrs,:), params);

% I now have the am and the bv, bvmat at each pixel
tstep=1; % always
nic=size(half_patch_width+1:tstep:nums-half_patch_width, 2);
njc=size(half_patch_width+1:tstep:numt-half_patch_width, 2);
%chicken!
ntiles=nums*numt;
nnz=size(ptrs, 1);
...
for i=1:nnz
  ...
  % TODO(jrock): This is slow, remove.
  assert(~any(isnan(wb2+wb1)) && ~any(isnan(wm(:))));

  wmbv(op:op+proj_dimensions-1)=wb2+wb1;
  % TODO(jrock): Might be able to remove this for loop.
...
b_img_transpose_filt = applytransposefilters(b_img, forest_model.filters);
b_img2 = reshape(reshape(b_img_transpose_filt, [], params.nchannels)', [], 1);
b_img2_masked = b_img2;%[];%b_img_transpose_filt(repmat(mask, [1,1,params.nchannels]));
