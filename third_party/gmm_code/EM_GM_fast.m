function [obj,W,M,V,L] = EM_GM_fast(X,k,ltol,maxiter,pflag,Init)
...
while (abs(100*(Ln-Lo)/Lo)>ltol) & (niter<=maxiter),
    ...
    [W,M] = Maximization(X,k,E);  % M-step
    Lo = Ln;
    Ln = Likelihood(X,k,W,M,V);
    niter = niter + 1;
    disp(['Iteration# ' num2str(niter) ' likelihood ' num2str(Ln)]);
end 
L = Ln;
obj = gmdistribution(M', V, W);
%disp('Output W:');
%W
%disp('Output M:');
%M
...
for i=1:k,
    ...
    %@aditya: Changed, don't modify variance
